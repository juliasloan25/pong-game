...
/**
 * A function that can be called on list elements to determine if their values
 * are equal. Returns a boolean.
 * Examples: vec_equal, NULL if not needed for list
 */
typedef bool (*EqualFunc)(void *data1, void *data2);
...
/**
* Sets the element at a given index in a list to a specified value.
*
* @param list a pointer to a list returned from list_init()
* @param index an index in the list (the first element is at 0)
...
/**
 * Checks if a list contains an element pointing to the same memory location as
 * a given value.
 *
 * @param list a pointer to a list returned from list_init()
 * @param value a pointer to a value to be compared
 * @return a boolean representing the presence of the value in the list
 */
bool list_contains(List *list, void* value);

/**
 * Returns the EqualFunc of a list
 *
 * @param list a pointer to a list returned from list_init()
 * @return the EqualFunc stored by the list
 */
