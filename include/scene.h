...
/**
* Struct that stores information regarding a force creator and its relevant pa-
* -parameters
*/
typedef struct Force_Param ForceParam;
 ...
 *
 * Removes and frees the body at a given index from a scene along with any force
 * attached to it.
 * Asserts that the index is valid.
 *
 * @param scene a pointer to a scene returned from scene_init()
...
void scene_add_bodies_force_creator(
    Scene *scene, ForceCreator forcer, void *aux, FreeFunc freer, List *bodies
);
